...
<a href="/borrows/create" class="btn btn-primary m-4">Buat Peminjaman</a>
        ...
        <tbody>
            @foreach ($borrows as $borrow)
                ...
                <tr class="{{ $isLate ? 'table-danger' : '' }}">
                    ...
                    <td>{{ $borrow->user->borrow ? $borrow->tgl_kembali : '-' }}</td>
                    <td>
                        <form class="" action="{{ route('borrows.destroy', $borrow->id) }}" method="post">
                            @csrf
                            @method('DELETE')
